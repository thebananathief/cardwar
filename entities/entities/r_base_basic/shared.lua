...
function ENT:FindEnemy()
	...
	local _ents = ents.FindInSphere(self:GetPos(), self.SearchRadius or 2048)
		for k,v in pairs(_ents) do
			if (type(v)=="NextBot") and v:Health() > 0 then
				if self:GetcwTeam() != v:GetcwTeam() then
					self:SetEnemy(v)
					return true
				end
			end
...
hook.Add( "PlayerCanPickupWeapon", "No_Pickup_NPC_Weapons", function( ply, wep )
	if type(wep:GetOwner())=="NextBot" then
		return false
	end
end )
