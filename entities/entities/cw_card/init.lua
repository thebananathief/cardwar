...
function ENT:Use(activator, caller)
    ...
    if ( par:IsValid() ) then
        if (table.HasValue(par.storage, self)) then
            timey = true
            timer.Simple(0.5, function() timey = false end)
            table.RemoveByValue(par.storage, self)
            if par:GetClass()=="cw_shelf" then par:SortCards() end
        end
        if (table.HasValue(loadoutRed, self)) then table.RemoveByValue(loadoutRed, self) end
        if (table.HasValue(loadoutBlue, self)) then table.RemoveByValue(loadoutBlue, self) end
    end
